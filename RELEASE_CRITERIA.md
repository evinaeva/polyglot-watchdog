...
The contract is normative for artifact semantics and phase boundaries.  
This file translates that into a release checklist.
...
### 7. Official operator workflow coherence

- The core v1.0 workflow may be distributed across multiple dedicated pages or tabs.
- A multi-page or tab-based operator experience is acceptable and is **not** a release blocker by itself.
- Core operator pages required for the v1.0 flow are not mock-backed in a way that breaks the required workflow.
- The operator can complete the required workflow through official product UI surfaces.
- The required workflow does not depend on hidden routes, developer-only paths, or manual intervention outside the product interface.
- Navigation between required pages is clear enough for an operator to complete the workflow intentionally.
- State required for moving between pages is persisted through the canonical product flow.

### 8. Documentation alignment

- README, About text, and truth-set docs do not contradict the actual implementation state.
- The multi-page/tab-based workflow model is described accurately if that is the intended product behavior.
...
- consolidating multiple operator steps into a single page, route, or screen
...
- a core operator route required for v1.0 remains mock-backed in a way that prevents completion of the documented workflow;
...
- the required workflow depends on hidden/manual developer steps not acknowledged in docs;
- the documentation incorrectly treats the intended multi-page workflow as an integration failure when it is actually the canonical product design.
...
- an operator-completable official workflow across the required pages/tabs;
...
- “operator-complete for the documented multi-page v1.0 workflow”
