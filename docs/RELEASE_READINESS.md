...
| 6 | Target-language comparison and issue generation | Phase 6 persists issues and explorer reads them; evidence drill-down exists | pass | `tests/test_phase6_schema_compliance.py`; `tests/test_issues_explorer_api.py`; `/api/issues/detail` via issues tooltip on `/` | blocker if not met | blocks production wording if fail/unknown |
