...
def test_smoke_before_after_with_same_fixtures(api_env):
    ...
    for path, status in before_pages.items():
        assert status not in disallowed_statuses, f"before: {path} returned {status}"
    for path, status in after_pages.items():
        assert status not in disallowed_statuses, f"after: {path} returned {status}"
    ...
    for route, keys in required_read_keys.items():
        ...
        assert before_status not in disallowed_statuses
        assert after_status not in disallowed_statuses
