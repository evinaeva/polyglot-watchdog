...
def test_pulls_runtime_removed_manual_prepare_controls_and_messages():
    source = Path("web/static/pulls.js").read_text(encoding="utf-8")
    assert "triggerEligibleDatasetGeneration" not in source
    assert "pullsPrepareCapturedData" not in source
    assert "pullsPrepareCapturedDataStatus" not in source
    assert "Captured data prepared successfully" not in source


def test_workflow_runtime_removed_refresh_and_debug_controls():
    source = Path("web/static/workflow.js").read_text(encoding="utf-8")
    assert "wfRefreshRuns" not in source
    assert "wfGenerateDataset" not in source
    assert "wfPayload" not in source
    assert "wfTransition" not in source
    assert "/api/workflow/generate-eligible-dataset" not in source
